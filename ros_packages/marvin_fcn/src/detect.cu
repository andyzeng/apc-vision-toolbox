...
#include "ros/ros.h"
#include "marvin_fcn/DetectObjects.h"
...
std::string shelf_net_arch_filename = "/home/andyz/apc/toolbox/ros_packages/catkin_ws/src/marvin_fcn/net/shelf_tote_testing.json";
std::string tote_net_arch_filename = "/home/andyz/apc/toolbox/ros_packages/catkin_ws/src/marvin_fcn/net/shelf_tote_testing.json";
std::string shelf_net_weights_filename = "/home/andyz/apc/toolbox/ros_packages/catkin_ws/src/marvin_fcn/net/shelf_net.marvin";
std::string tote_net_weights_filename = "/home/andyz/apc/toolbox/ros_packages/catkin_ws/src/marvin_fcn/net/tote_net.marvin";
...
// Service call
bool srv_detect(marvin_fcn::DetectObjects::Request  &req,
                marvin_fcn::DetectObjects::Response &res) {
  ...
  // Call ROS service to compute HHA maps from depth frames
  ...
  if (service_mode == READ_SENSOR_FORWARD_PASS || service_mode == READ_FILE_FORWARD_PASS) {
    ...
    for (int r = 0; r < frame_height; ++r)
      for (int c = 0; c < frame_width; ++c) {
        color_data_CPU[0 * frame_height * frame_width + r * frame_width + c] = CPUCompute2StorageT(ComputeT(cloud_buffer_rgb[2 * frame_height * frame_width + r * frame_width + c]) - ComputeT(102.9801f)); // B
        ...
        color_data_CPU[2 * frame_height * frame_width + r * frame_width + c] = CPUCompute2StorageT(ComputeT(cloud_buffer_rgb[0 * frame_height * frame_width + r * frame_width + c]) - ComputeT(122.7717f)); // R
      } 
...
int main(int argc, char **argv) {
  ...
  // Setup ROS
  ros::init(argc, argv, "marvin_fcn", ros::init_options::AnonymousName);
  ros::NodeHandle n;
  ros::NodeHandle priv_nh("~");

  // Get service parameters
  priv_nh.param("service_name", service_name, std::string("marvin_fcn"));
