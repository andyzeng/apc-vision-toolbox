function resultImage = visualizeResults(RtPCA,latentPCA,surfaceCentroid,surfaceRange,finalRt,finalScore,dataPath,objName,instanceIdx,binID,data,objSegPts,objMasks,objModelPts)
...
for baseFrameIdx = frames
  segImage = data.color{baseFrameIdx};
  extBin2Cam = inv(data.extCam2World{baseFrameIdx})*data.extBin2World;
  if strcmp(objName,'rolodex_jumbo_pencil_cup') || strcmp(objName,'dasani_water_bottle') || strcmp(objName,'platinum_pets_dog_bowl') || strcmp(objName,'easter_turtle_sippy_cup')
    ...
    pixX = round(((camObjSegPts(1,:).*data.K(1,1))./camObjSegPts(3,:))+data.K(1,3));
    pixY = round(((camObjSegPts(2,:).*data.K(2,2))./camObjSegPts(3,:))+data.K(2,3));
  ...
  modelRt = extBin2Cam * data.extWorld2Bin * finalRt;
  RtPCA = data.extWorld2Bin * RtPCA;
  if strcmp(objName,'cherokee_easy_tee_shirt') || strcmp(objName,'kyjen_squeakin_eggs_plush_puppies') || strcmp(objName,'womens_knit_gloves') || strcmp(objName,'cloud_b_plush_bear') || ...
    ...
    extBin2Obj = inv(data.extWorld2Bin * modelRt);
    objModelPts = extBin2Obj(1:3,1:3) * objSegPts + repmat(extBin2Obj(1:3,4),1,size(objSegPts,2));

    modelRt = extBin2Cam * data.extWorld2Bin * modelRt;
  ...
  pixX = round(((camObjModelPts(1,:).*data.K(1,1))./camObjModelPts(3,:))+data.K(1,3));
  pixY = round(((camObjModelPts(2,:).*data.K(2,2))./camObjModelPts(3,:))+data.K(2,3));
  validPix = find(pixX > 0 & pixX <= 640 & pixY > 0 & pixY <= 480);
  objMask = zeros(480,640);
  objMask(sub2ind(size(objMask),pixY(validPix)',pixX(validPix)')) = 1;

  objImage = data.color{baseFrameIdx};
  if drawModel
  ...
  axisPixX = round(((camAxisPts(1,:).*data.K(1,1))./camAxisPts(3,:))+data.K(1,3));
  axisPixY = round(((camAxisPts(2,:).*data.K(2,2))./camAxisPts(3,:))+data.K(2,3));
  ...
  if drawBbox
    ...
    cornerPixX = round(((camCornerPts(1,:).*data.K(1,1))./camCornerPts(3,:))+data.K(1,3));
    cornerPixY = round(((camCornerPts(2,:).*data.K(2,2))./camCornerPts(3,:))+data.K(2,3));
...
imwrite(allObjImage,fullfile(dataPath,strcat(objName,sprintf('.%d.result.png',instanceIdx))));
